...
const FALLBACK_CV = {
  ...
  website: 'alexp.dev',
  summary: 'Опитен frontend инженер с 6+ години опит в изграждането на мащабируеми уеб приложения.',
...
function normalizeCv(cv) {
  ...
  const name = cv.name || cv.fullName || cv.full_name || 'Без ime'
  ...
  return `
    <main class="cv nova">
      <aside class="sidebar">
        ...
        <section><h2>Контакти</h2>${renderContact(data)}</section>
        <section><h2>Умения</h2><div class="skill-list">${listItems(data.skills, 'skill')}</div></section>
        <section><h2>Езици</h2>${renderLanguages(data)}</section>
      </aside>
      <section class="content">
        <section class="block"><h2>Профил</h2><p>${escapeHtml(data.summary)}</p></section>
        <section class="block"><h2>Опит</h2>${renderExperience(data)}</section>
        <section class="block"><h2>Образование</h2>${renderEducation(data)}</section>
        <section class="block"><h2>Сертификати</h2><div class="pill-list">${renderCertifications(data)}</div></section>
      </section>
    </main>
  `
}
    ...
    <main class="cv onyx">
      ...
      <div class="onyx-grid">
        ...
        <aside class="side-panel">
          <section class="block"><h2>Умения</h2><div class="skill-list">${listItems(data.skills, 'skill')}</div></section>
          <section class="block"><h2>Образование</h2>${renderEducation(data)}</section>
          <section class="block"><h2>Езици</h2>${renderLanguages(data)}</section>
          <section class="block"><h2>Сертификати</h2><div class="pill-list">${renderCertifications(data)}</div></section>
        </aside>
      </div>
    </main>
...
function renderSlateHtml(data) {
  return `
    <main class="cv slate">
      ...
      <section class="block"><h2>Резюме</h2><p>${escapeHtml(data.summary)}</p></section>
      <section class="block"><h2>Трудов стаж</h2>${renderExperience(data)}</section>
      <section class="block"><h2>Образование</h2>${renderEducation(data)}</section>
      <section class="two-cols">
        <div class="block"><h2>Технологии</h2><div class="pill-list">${listItems(data.skills, 'pill')}</div></div>
        <div class="block"><h2>Езици</h2>${renderLanguages(data)}</div>
      </section>
    </main>
  `
}
...
function getExportStyles(templateId) {
  return `
    * { box-sizing: border-box; }
    body { margin: 0; background: #e2e8f0; color: #1e293b; font-family: 'Plus Jakarta Sans', Inter, system-ui, sans-serif; line-height: 1.6; }
    .cv { width: min(1100px, 100%); min-height: 100vh; margin: 0 auto; background: #fff; box-shadow: 0 24px 80px rgba(15,23,42,0.18); }
    h1, h2, h3, p { margin: 0; }
    ...
    .avatar { width: 78px; height: 78px; border-radius: 22px; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: 900; flex-shrink: 0; }
    ...
    <div style={{ display: 'flex', minHeight: '100%', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontSize: 13, lineHeight: 1.55, color: '#1e293b', background: '#fff' }}>
      <div style={{ width: 220, flexShrink: 0, background: 'linear-gradient(180deg, #1e3a8a 0%, #2563eb 100%)', color: '#fff', padding: '32px 20px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.75)', fontWeight: 500 }}>{data.title}</div>
        </div>
        <div>
          <SbHead label={t('templates.cv_section_contacts')} />
          {[{ icon: HiMail, val: data.email }, { icon: HiPhone, val: data.phone }, { icon: HiLocationMarker, val: data.location }, { icon: HiExternalLink, val: data.website }]
        ...
        {data.skills.length > 0 && (
          <div>
            <SbHead label={t('templates.cv_section_skills')} />
        ...
        {data.languages.length > 0 && (
          <div>
            <SbHead label={t('templates.cv_section_languages')} />
      ...
      <div style={{ flex: 1, padding: '32px 28px', overflowY: 'auto' }}>
        <section style={{ marginBottom: 24 }}>
          <MH label={t('templates.cv_section_profile')} color="#2563eb" />
          <p style={{ fontSize: 12.5, color: '#475569', lineHeight: 1.7 }}>{data.summary}</p>
        </section>
        {data.experience.length > 0 && (
          <section style={{ marginBottom: 24 }}>
            <MH label={t('templates.cv_section_experience')} color="#2563eb" />
        ...
        {data.education.length > 0 && (
          <section style={{ marginBottom: 24 }}>
            <MH label={t('templates.cv_section_education')} color="#2563eb" />
        ...
        {data.certifications.length > 0 && (
          <section>
            <MH label={t('templates.cv_section_certifications')} color="#2563eb" />
    ...
    <div style={{ background: bg, minHeight: '100%', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontSize: 13, color: '#e2e8f0' }}>
            ...
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {[data.email, data.phone, data.location].filter(Boolean).map(v => (
                <span key={v} style={{ fontSize: 10.5, color: muted, display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span style={{ width: 4, height: 4, borderRadius: '50%', background: gold, display: 'inline-block' }} />{v}
                </span>
              ))}
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 0 }}>
        <div style={{ padding: '28px 32px', borderRight: `1px solid rgba(255,255,255,0.07)` }}>
          <OnyxSection label={t('templates.cv_section_profile')} gold={gold}>
            <p style={{ fontSize: 12, color: muted, lineHeight: 1.75 }}>{data.summary}</p>
          </OnyxSection>
          {data.experience.length > 0 && (
            <OnyxSection label={t('templates.cv_section_experience')} gold={gold}>
              {data.experience.map((e, i) => (
        ...
        <div style={{ padding: '28px 24px', background: card }}>
          {data.skills.length > 0 && (
            <OnyxSection label={t('templates.cv_section_skills')} gold={gold}>
                ...
                {data.skills.map((s, i) => (
                  <div key={s}>
                    ...
                    <div style={{ height: 3, background: 'rgba(255,255,255,0.1)', borderRadius: 2 }}>
                      <div style={{ height: '100%', borderRadius: 2, background: `linear-gradient(90deg, ${gold}, ${gold}88)`, width: `${60 + (i * 13) % 35}%` }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {data.education.length > 0 && (
            <OnyxSection label={t('templates.cv_section_education')} gold={gold}>
              {data.education.map((e, i) => (
                <div key={i}>
          ...
          {data.languages.length > 0 && (
            <OnyxSection label={t('templates.cv_section_languages')} gold={gold}>
              {data.languages.map(l => (
          ...
          {data.certifications.length > 0 && (
            <OnyxSection label={t('templates.cv_section_certifications')} gold={gold}>
              {data.certifications.map(c => (
...
function SlateTemplate({ cv, t }) {
  ...
  return (
    <div style={{ background: '#fff', minHeight: '100%', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", maxWidth: 680, margin: '0 auto', padding: '44px 48px', fontSize: 13, color: '#1e293b' }}>
      ...
      <SlateSection label={t('templates.cv_section_profile')}>
        <p style={{ fontSize: 12.5, color: '#475569', lineHeight: 1.75 }}>{data.summary}</p>
      </SlateSection>
      {data.experience.length > 0 && (
        <SlateSection label={t('templates.cv_section_experience')}>
          {data.experience.map((e, i) => (
      ...
      {data.education.length > 0 && (
        <SlateSection label={t('templates.cv_section_education')}>
          {data.education.map((e, i) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        {data.skills.length > 0 && (
          <SlateSection label={t('templates.cv_section_skills')}>
        ...
        {data.languages.length > 0 && (
          <SlateSection label={t('templates.cv_section_languages')}>
            {data.languages.map(l => (
    ...
    <div style={{ background: '#f0fdf4', minHeight: '100%', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontSize: 13, color: '#1e293b' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 220px', gap: 0 }}>
        <div style={{ padding: '28px 32px', borderRight: '1px solid #d1fae5' }}>
          <AuSection label={t('templates.cv_section_profile')} teal={teal}>
            <p style={{ fontSize: 12.5, color: '#475569', lineHeight: 1.75 }}>{data.summary}</p>
          </AuSection>
          {data.experience.length > 0 && (
            <AuSection label={t('templates.cv_section_experience')} teal={teal}>
              {data.experience.map((e, i) => (
        ...
        <div style={{ padding: '28px 20px', background: '#f0fdf4' }}>
          {data.education.length > 0 && (
            <AuSection label={t('templates.cv_section_education')} teal={teal}>
              {data.education.map((e, i) => (
          ...
          {data.languages.length > 0 && (
            <AuSection label={t('templates.cv_section_languages')} teal={teal}>
              {data.languages.map(l => (
          ...
          {data.certifications.length > 0 && (
            <AuSection label={t('templates.cv_section_certifications')} teal={teal}>
              {data.certifications.map(c => (
...
/* ─── CV picker modal ─────────────────────────────────────────── */
function CvPickerModal({ cvs, selectedCv, activeTemplate, onSelect, onClose, t }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.58)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 560, background: 'var(--white)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-xl)', padding: 24, position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 14, width: 34, height: 34, borderRadius: 10, border: '1px solid var(--border)', background: 'var(--white)', color: 'var(--ink-50)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.25rem', fontWeight: 800, color: 'var(--ink)', marginBottom: 8 }}>
          {t('templates.pick_cv_title')}
        </h2>
        <p style={{ fontSize: '0.875rem', color: 'var(--ink-60)', marginBottom: 18, lineHeight: 1.6 }}>
          {t('templates.pick_cv_subtitle', { name: activeTemplate.name })}
        </p>

        {!cvs || cvs.length === 0 ? (
          <div style={{ padding: 18, borderRadius: 'var(--radius-lg)', background: 'var(--canvas)', border: '1px solid var(--border)', textAlign: 'center' }}>
            <p style={{ color: 'var(--ink-60)', marginBottom: 14 }}>{t('templates.no_cvs')}</p>
            <Link to="/analyze" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 'var(--radius)', background: 'var(--brand)', color: '#fff', fontWeight: 700, textDecoration: 'none' }}>
              <HiChartBar style={{ width: 14, height: 14 }} />
              {t('templates.no_cvs_link')}
            </Link>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 380, overflowY: 'auto' }}>
            {cvs.map(cv => {
              ...
              return (
                <button key={cv.id || `${data.name}-${data.email}`} onClick={() => onSelect(cv)} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: 14, borderRadius: 'var(--radius-lg)', border: `1px solid ${isSelected ? activeTemplate.accent : 'var(--border)'}`, background: isSelected ? `${activeTemplate.accent}10` : 'var(--white)', cursor: 'pointer', textAlign: 'left' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '0.78rem', color: 'var(--ink-50)', marginTop: 2 }}>{data.title || data.email || t('jobs.saved_cv')}</div>
                    {data.email && <div style={{ fontSize: '0.72rem', color: 'var(--ink-40)', marginTop: 3 }}>{data.email}</div>}
                  </div>
                  {isSelected && <HiCheck style={{ width: 18, height: 18, color: activeTemplate.accent, flexShrink: 0 }} />}
                </button>
              )
            })}
          </div>
...
function TemplatePreview({ id, cv, t }) {
  switch (id) {
    case 'onyx':   return <OnyxTemplate cv={cv} t={t} />
    case 'slate':  return <SlateTemplate cv={cv} t={t} />
    case 'aurora': return <AuroraTemplate cv={cv} t={t} />
    default:       return <NovaTemplate cv={cv} t={t} />
  }
}
...
function Templates() {
  ...
  const TEMPLATES = [
    { id: 'nova',   name: 'Nova',   tag: t('templates.template_nova_tag'),   tagColor: '#2563eb', desc: t('templates.template_nova_desc'),   accent: '#2563eb', industries: t('templates.template_nova_industries') },
    { id: 'onyx',   name: 'Onyx',   tag: t('templates.template_onyx_tag'),   tagColor: '#c9a84c', desc: t('templates.template_onyx_desc'),   accent: '#c9a84c', industries: t('templates.template_onyx_industries'), dark: true },
    { id: 'slate',  name: 'Slate',  tag: t('templates.template_slate_tag'),  tagColor: '#475569', desc: t('templates.template_slate_desc'),  accent: '#0f172a', industries: t('templates.template_slate_industries') },
    { id: 'aurora', name: 'Aurora', tag: t('templates.template_aurora_tag'), tagColor: '#059669', desc: t('templates.template_aurora_desc'), accent: '#0d9488', industries: t('templates.template_aurora_industries') },
  ]
  ...
  function handleDownloadHtml() {
    if (!selectedCv) {
      setDownloadError(t('templates.select_cv_first'))
      setShowCvModal(true)
      return
    }
    setDownloadError('')
    ...
    <div className="page-container" style={{ paddingTop: 'clamp(40px,6vw,68px)', paddingBottom: 'clamp(60px,8vw,100px)' }}>
      ...
      <div className="templates-layout" style={{ display: 'grid', gridTemplateColumns: 'clamp(260px,30%,320px) 1fr', gap: 24, alignItems: 'start' }}>
        <div className="templates-sidebar" style={{ display: 'flex', flexDirection: 'column', gap: 8, position: 'sticky', top: 88 }}>
          ...
          {TEMPLATES.map(tmpl => (
            <button key={tmpl.id} onClick={() => setActive(tmpl.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px', borderRadius: 'var(--radius-lg)', border: `2px solid ${active === tmpl.id ? tmpl.accent : 'var(--border)'}`, background: active === tmpl.id ? (tmpl.dark ? `${tmpl.accent}18` : `${tmpl.accent}07`) : 'var(--white)', cursor: 'pointer', textAlign: 'left', transition: 'border-color 0.15s, background 0.15s, box-shadow 0.15s', boxShadow: active === tmpl.id ? `0 0 0 3px ${tmpl.accent}18, var(--shadow-md)` : 'var(--shadow-xs)' }}>
              <Swatch id={tmpl.id} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                  <span style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--ink)' }}>{tmpl.name}</span>
                  <span style={{ fontSize: '0.62rem', fontWeight: 700, padding: '2px 7px', borderRadius: 99, background: `${tmpl.tagColor}14`, color: tmpl.tagColor }}>{tmpl.tag}</span>
                </div>
                <p style={{ fontSize: '0.75rem', color: 'var(--ink-40)', lineHeight: 1.5, margin: 0 }}>{tmpl.desc}</p>
                <p style={{ fontSize: '0.68rem', color: 'var(--ink-40)', marginTop: 4 }}>
                  <span style={{ color: tmpl.accent, fontWeight: 600 }}>✦</span> {tmpl.industries}
                </p>
              </div>
              {active === tmpl.id && (
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: tmpl.accent, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ marginTop: 8, padding: '18px 16px', borderRadius: 'var(--radius-lg)', background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 60%, #2563eb 100%)', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '0.8125rem', fontWeight: 700, color: '#fff', marginBottom: 5 }}>{t('templates.ai_ready_title')}</p>
            <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.6)', marginBottom: 14, lineHeight: 1.55 }}>
              {t('templates.ai_ready_desc')}
            </p>
          ...
          <div style={{ borderRadius: 'var(--radius-xl)', overflow: 'hidden', border: '1px solid var(--border)', boxShadow: 'var(--shadow-xl)', background: '#fff' }}>
            ...
            <div style={{ height: 600, overflow: 'hidden', position: 'relative', background: activeT.dark ? '#141414' : '#f8fafc' }}>
              <div style={{ width: '167%', height: '167%', transformOrigin: 'top left', transform: 'scale(0.6)', overflowY: 'auto', overflowX: 'hidden' }}>
                <TemplatePreview id={active} cv={previewCv} t={t} />
              </div>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <button onClick={() => setShowCvModal(true)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '12px 16px', borderRadius: 'var(--radius)', background: activeT.accent, color: '#fff', fontWeight: 700, fontSize: '0.875rem', border: 'none', cursor: 'pointer', fontFamily: 'var(--font)', transition: 'opacity 0.15s, transform 0.15s', boxShadow: `0 4px 16px ${activeT.accent}40` }}
              onMouseOver={e => { e.currentTarget.style.opacity = '0.88'; e.currentTarget.style.transform = 'translateY(-1px)' }}
              onMouseOut={e => { e.currentTarget.style.opacity = '1'; e.currentTarget.style.transform = 'translateY(0)' }}
            >
              <HiSparkles style={{ width: 14, height: 14 }} />
              {selectedCv ? t('templates.change_cv') : t('templates.use_template')}
            </button>

            <button onClick={handleDownloadHtml} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '12px 16px', borderRadius: 'var(--radius)', background: 'var(--white)', color: selectedCv ? 'var(--ink)' : 'var(--ink-40)', fontWeight: 700, fontSize: '0.875rem', border: '1px solid var(--border)', cursor: 'pointer', fontFamily: 'var(--font)', opacity: selectedCv ? 1 : 0.75 }}>
              <HiDownload style={{ width: 14, height: 14 }} />
              {t('templates.download_html')}
            </button>
          </div>
      ...
      <div style={{ marginTop: 64 }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.2rem,3vw,1.5rem)', fontWeight: 700, color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: 6 }}>
          {t('templates.all_templates_title')}
        </h2>
        <p style={{ fontSize: '0.9rem', color: 'var(--ink-60)', marginBottom: 28 }}>{t('templates.all_templates_desc')}</p>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,280px), 1fr))', gap: 20 }}>
          {TEMPLATES.map(tmpl => (
            <div key={tmpl.id} onClick={() => { setActive(tmpl.id); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
              style={{ borderRadius: 'var(--radius-xl)', overflow: 'hidden', cursor: 'pointer', border: `2px solid ${active === tmpl.id ? tmpl.accent : 'var(--border)'}`, boxShadow: active === tmpl.id ? `0 0 0 3px ${tmpl.accent}18, var(--shadow-lg)` : 'var(--shadow-sm)', transition: 'all 0.2s var(--ease-out)' }}
              onMouseOver={e => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = `var(--shadow-xl)` }}
              onMouseOut={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = active === tmpl.id ? `0 0 0 3px ${tmpl.accent}18, var(--shadow-lg)` : 'var(--shadow-sm)' }}
            >
              <div style={{ height: 220, overflow: 'hidden', position: 'relative', background: tmpl.dark ? '#141414' : '#f8fafc' }}>
                <div style={{ width: '200%', height: '200%', transformOrigin: 'top left', transform: 'scale(0.5)', pointerEvents: 'none' }}>
                  <TemplatePreview id={tmpl.id} cv={previewCv} t={t} />
                </div>
                {active === tmpl.id && (
                  <div style={{ position: 'absolute', top: 10, right: 10, background: tmpl.accent, borderRadius: 99, padding: '3px 10px', fontSize: '0.65rem', fontWeight: 700, color: '#fff' }}>
                    {t('templates.active')}
                  </div>
                )}
              </div>
              <div style={{ padding: '12px 14px', background: 'var(--white)', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <span style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--ink)' }}>{tmpl.name}</span>
                  <span style={{ marginLeft: 7, fontSize: '0.62rem', fontWeight: 700, padding: '2px 7px', borderRadius: 99, background: `${tmpl.tagColor}14`, color: tmpl.tagColor }}>{tmpl.tag}</span>
                </div>
                <HiColorSwatch style={{ width: 14, height: 14, color: tmpl.accent }} />
              </div>
            </div>
          ))}
        </div>
      ...
      {showCvModal && (
        <CvPickerModal cvs={cvs} selectedCv={selectedCv} activeTemplate={activeT} onSelect={handleSelectCv} onClose={() => setShowCvModal(false)} t={t} />
      )}
    </div>
  )
}
